...
public class AI{
	...
	private void start(){
		...
		float S = 0.0f;
		for(float f:output) S += f;
		float[] prob = new float[output.length-1];
		...
		ans = 0;
		for(float f:prob) if(choice > f) ans++;
	}
	...
	public void backPropagation(float[] corrections){
		for(int i = 0; i < outputIds.length; i++){
			...
			Ns[outputIds[i]].backPropagate(2.0f/divisor*(Ns[outputIds[i]].getValue(false) - corrections[i]));
