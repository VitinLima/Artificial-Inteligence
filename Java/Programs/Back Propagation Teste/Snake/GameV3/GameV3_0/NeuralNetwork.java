public class NeuralNetwork{
	...
	public class Neuron{
		...
		public float fget_value(){
			/*if(is_receptor)
				System.out.println("neuron " + id + " is a receptor");
			if(is_active)
				System.out.println("neuron " + id + " is already active");*/
			if(is_receptor || is_active) return value;
			//System.out.println("activating neuron " + id);
			is_active = true;
			float new_value = 0.0f;
			for(Dentrite dentrite:dentrites)
				new_value += dentrite.fget_value();
			new_value += bias;
			new_value = Sigmoid(value);
			is_active = false;
			//System.out.println("deactivating neuron " + id);
			value = new_value;
			return value;
		}
		...
		private class Dentrite{
			...
			public void fmodify(float increment){
				...
				multiplier -= increment*neurons[axon_id].fget_simple_value();
			}
		}
	}
