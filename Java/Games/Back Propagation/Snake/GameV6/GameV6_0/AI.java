public class AI{
	...
	public AI(int n_receivers, int n_mem, int n_transmitters){
		...
		input_ids = new int[n_receivers];
		Ns = new Neuron[n_receivers + n_mem + n_transmitters];
		
		for(int i = 0; i < Ns.length; i++)
			Ns[i] = new Neuron(i);
		
		for(int i = 0; i < input_ids.length; i++) input_ids[i] = k++;
		for(int i = 0; i < output_ids.length; i++) output_ids[i] = k++;
		
		for(int id_out:output_ids) for(int id_in:input_ids)
			Ns[id_out].fconnect_to(Ns[id_in]);
		
		int temp;
		for(int i = input_ids.length; i < Ns.length; i++){
			temp = (int)((float)Math.random()*(float)(Ns.length - 1) + 1.0f);
			for(int j = 0; j < temp; j++)
				Ns[i].fconnect_to(Ns[(int)((float)Math.random()*Ns.length)]);
		}
