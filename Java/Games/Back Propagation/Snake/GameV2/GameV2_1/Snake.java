...
public class Snake{
	...
	public Color fmove(Color[][] grid){
		...
		switch(opt){
			case 0:
				temp_x++;
				if(temp_x == grid.length) temp_x = 0;
				break;
			case 1:
				temp_x--;
				if(temp_x == -1) temp_x = grid.length-1;
				break;
			case 2:
				temp_y++;
				if(temp_y == grid.length) temp_y = 0;
				break;
			case 3:
				temp_y--;
				if(temp_y == -1) temp_y = grid.length-1;
				break;
		}
		Color event;
		if(temp_x < 0 || temp_x >= grid.length || temp_y < 0 || temp_y >= grid[0].length)
			event = Color.black;
		else
