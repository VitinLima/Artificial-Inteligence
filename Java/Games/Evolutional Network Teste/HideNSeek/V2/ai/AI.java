	...
	public void backPropagate(boolean incentive){
		...
		/*if(incentive){
			for(int j = 0; j < corrections.length; j++)
				corrections[j] = Ns[outputIds[j]].getValue(false) + (float)Math.cos(2.0*Math.PI*(double)Ns[outputIds[j]].getValue(false))/2.0f;
		} else{
			for(int j = 0; j < corrections.length; j++)
				corrections[j] = Ns[outputIds[j]].getValue(false) + (float)Math.sin(Math.PI*(double)Ns[outputIds[j]].getValue(false))/2.0f;
		}*/
		if(incentive){
			for(int j = 0; j < corrections.length; j++)
				corrections[j] = 0.0f;
