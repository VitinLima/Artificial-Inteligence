...
public class SeekerUser extends Seeker{
	private UserFrame frame;
	private boolean seeing;
	private double[] angles;
	private int definition = 8;
	
	public SeekerUser(float p0, float p1, float p2, float p3){
		super(p0,p1,p2,p3);
	...
	public void step(){
		...
		for(int i = 0; i < definition; i++){
			input[i] = 0.0f;
			if(view[i] != null){
				if(view[i] instanceof Hider)
					input[i] = -1.0f;
				else if(view[i] instanceof Seeker)
					input[i] = 1.0f;
				else if(view[i] instanceof Wall){
					if(view[i].getOwner() instanceof Hider)
						input[i] = -0.5f;
					else
						input[i] = 0.5f;
				}
