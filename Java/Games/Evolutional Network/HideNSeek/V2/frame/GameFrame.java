...
public class GameFrame extends JFrame{
	...
	public void refresh(Entity[] es){
		...
		for(Entity e:es){
			...
			else if(e instanceof Wall || e instanceof Border)
			...
			if(e.getOwner() instanceof Hider){
				g.setColor(Color.blue);
				g.drawLine((int)e.getX()-(int)e.getSize()/2, (int)e.getY(), (int)e.getX()+(int)e.getSize()/2-1, (int)e.getY());
				g.drawLine((int)e.getX(), (int)e.getY()-(int)e.getSize()/2, (int)e.getX(), (int)e.getY()+(int)e.getSize()/2-1);
