...
public class Snake extends Entity{
	...
	public Snake(Color new_color, Game_Object new_obj){
		...
		field_of_view = 1;
		ai = new AI(new int[]{2*(2*field_of_view+1)*(2*field_of_view+1), 4}, 2*field_of_view);
	...
	private boolean fdeath_event(int x, int y){
		...
		if(fget_obj().fget_thread(cur_id).getName().contains("snake")){
		...
		for(int i = x - field_of_view; i <= x + field_of_view; i++){
			for(int j = y - field_of_view; j <= y + field_of_view; j++){
				if(i < 0 || i >= fget_obj().fget_map().fget_size_x() || j < 0 || j >= fget_obj().fget_map().fget_size_y()){
					...
					cur_id = fget_obj().fget_map().fget_id(i, j);
					if(cur_id == 0){
						input[(2*field_of_view+1)*(2*field_of_view+1) + k] = 0.0f;
						input[k++] = 0.0f;
					} else{
						if(fget_obj().fget_thread(cur_id).getName().contains("fruit")){
